...
def split_string_with_slash_and_take_last(string: str):
    """
    Splits a string by '/' and takes the last part.
    """
    if not isinstance(string, str):
        raise ValueError(f"Expected a string, got {type(string)}")

    parts = string.split("/")
    if len(parts) == 0:
        raise ValueError("String does not contain any '/' characters.")
...
OmegaConf.register_new_resolver("split_string_with_slash_and_take_last", split_string_with_slash_and_take_last)
